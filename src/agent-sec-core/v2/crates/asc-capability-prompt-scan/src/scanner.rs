...
impl PromptScanner {
    ...
    ///
    /// Inputs exceeding `MAX_INPUT_BYTES` (1 MiB) are truncated to that limit
    /// on a UTF-8 character boundary before scanning; the result records the
...
#[cfg(test)]
mod tests {
    // The float assertions below pin exact values on purpose: the latency
    // identity (`elapsed == init + scan`) and the confidence thresholds are the
    // contract, and every asserted value is exactly representable.
    ...
    impl ModelClient for FakeClient {
        ...
        ) -> Result<Value, asc_model_client::ModelClientError> {
    ...
    impl ModelClient for DownClient {
        ...
        fn generate(
            ...
            Err(asc_model_client::ModelClientError::Inference("down".into()))
        }
        fn chat(
            &self,
            _m: &str,
    ...
    fn oversized_input_is_truncated_and_flagged() {
        ...
        let bytes_scanned = result.metadata["input_bytes_scanned"].as_u64().unwrap_or(0);
        assert!(bytes_scanned <= MAX_INPUT_BYTES as u64);
