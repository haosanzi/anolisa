...
#[cfg(test)]
mod tests {
    // The float assertions below pin exact values on purpose: `round_py`'s
    // banker's-rounding result and the confidence thresholds are the contract,
    // and every asserted value is exactly representable.
