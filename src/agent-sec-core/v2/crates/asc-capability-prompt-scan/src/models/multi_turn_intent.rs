...
use asc_model_client::{
    GenerateRequest, ModelClient, ModelClientConfig, ModelOptions, OllamaClient,
};
...
impl MultiTurnIntentClassifier {
    ...
    pub fn new(harmful_threshold: f64) -> Result<Self, ScannerError> {
        Ok(MultiTurnIntentClassifier {
            ...
            client: Box::new(OllamaClient::from_config(&ModelClientConfig::from_env()?)),
        })
    }
...
#[cfg(test)]
mod tests {
    // The float assertions below pin exact values on purpose: the softmax
    // reconstruction and the fallback constants are part of this module's
