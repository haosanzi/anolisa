...
pub enum ScannerError {
    ...
    /// Wraps an upstream [`asc_model_client::ModelClientError`]; produced by
    /// `?` propagation from the shared model client.
    #[error("model client error: {0}")]
    ModelClient(#[from] asc_model_client::ModelClientError),
}
