...
/// measured (not the schema): the schema enum is `mcp`/`skill`/`both`/
/// `runtime`, but stable rules universally omit the top-level field — serde
/// defaults it to the empty string, hence "" is whitelisted. Observed
/// input-surface values (`mcp`/`both`/`llm_io`/`llm`) stay; `skill`,
/// `runtime` and `tool_*` surfaces are excluded. Re-verify on every
/// upstream bump.
const SCAN_TARGETS: &[&str] = &["", "mcp", "both", "llm_io", "llm"];

/// Pattern-level condition fields carrying user prompt input. Conditions
/// on other fields (`tool_response`, `agent_output`, `tool_args`, ...)
/// address non-input surfaces and are skipped with a reported reason.
...
/// Parse and validate the CLI arguments into `(atr_dir, tag)`.
///
/// # Errors
///
/// Fails on an unknown or valueless argument, a missing required argument,
/// a tag outside `[A-Za-z0-9._-]`, or an `--atr-dir` that is not an ATR
/// checkout.
...
/// Write `rules/atr/UPSTREAM.toml`: the sync provenance plus every
/// exclusion and its reason.
///
/// # Errors
///
/// Fails if the report cannot be written to `out_dir`.
fn write_report(
    out_dir: &Path,
    tag: &str,
    commit: &str,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_report(&out_dir, &tag, &commit, kept_total, &skips)?;
    println!(
